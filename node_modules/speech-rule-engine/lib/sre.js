...
c);return c}sre.Debugger.getInstance().generateOutput(goog.bind(function(){return["Apply Rule:",d.name,d.dynamicCstr.toString(),b.mode!==sre.Engine.Mode.HTTP?a.toString():a]},this));var e=d.context||this.activeStore_.context,f=d.action.components;c=[];for(var g=0,h;h=f[g];g++){var k=[],l=h.content||"",m=h.attributes||{},n=!1;h.grammar&&this.processGrammar(e,a,h.grammar);var p=null,t=null;if(m.engine){p=sre.Engine.getInstance().dynamicCstr.getComponents();var u=sre.Grammar.parseInput(m.engine);t=this.cache_;
this.clearCache();sre.Engine.getInstance().setDynamicCstr(u)}switch(h.type){case sre.SpeechRule.Type.NODE:(l=e.applyQuery(a,l))&&(k=this.evaluateTree_(l));break;case sre.SpeechRule.Type.MULTI:n=!0;l=e.applySelector(a,l);0<l.length&&(k=this.evaluateNodeList_(e,l,m.sepFunc,e.constructString(a,m.separator),m.ctxtFunc,e.constructString(a,m.context)));break;case sre.SpeechRule.Type.TEXT:(l=e.constructString(a,l))&&(k=[sre.AuditoryDescription.create({text:l},{adjust:!0})]);break;default:k=[sre.AuditoryDescription.create({text:l})]}k[0]&&
!n&&(m.context&&(k[0].context=e.constructString(a,m.context)+(k[0].context||"")),m.annotation&&(k[0].annotation=m.annotation));h.grammar&&sre.Grammar.getInstance().popState();c=c.concat(this.addPersonality_(k,m,n,a));p&&(this.cache_=t,sre.Engine.getInstance().setDynamicCstr(p))}this.pushCache_(a,c);return c};
...
sre.Cli.prototype.enumerate=function(){this.system.setupEngine(this.setup);var a=sre.DynamicCstr.DEFAULT_ORDER.map(function(q){return q.length}),b=function(q,v){a[v]=Math.max.apply(null,Object.keys(q).map(function(w){return w.length}).concat(a[v]))},c=function(q,v){return q+Array(v-q.length+1).join(" ")},d=sre.SpeechRuleEngine.getInstance().enumerate();d=sre.MathCompoundStore.getInstance().enumerate(d);var e=[];b(d,0);for(var f in d){var g=!0,h=d[f];b(h,1);for(var k in h){var l=!0,m=h[k];b(m,2);for(var n in m){var p=
Object.keys(m[n]).sort();if("clearspeak"===n){p=!0;var t=sre.ClearspeakPreferences.getLocalePreferences(d)[f],u;for(u in t)e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(p?n:"",a[2]),t[u].join(", ")]),p=!1}else e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(n,a[2]),p.join(", ")]);l=g=!1}}}var x=0,r="";r+=sre.DynamicCstr.DEFAULT_ORDER.slice(0,-1).map(function(q){return c(q,a[x++])}).join(" | ");r+="\n";a.forEach(function(q){r+=Array(q+3).join("=")});r+="========================\n";r+=e.map(function(q){return q.join(" | ")}).join("\n");
console.info(r)};sre.Cli.prototype.execute=function(a){var b=sre.SystemExternal.commander;this.runProcessors_(goog.bind(function(c,d){this.system.processFile(c,d,b.output)},this),a)};sre.Cli.prototype.runProcessors_=function(a,b){try{this.processors.length||this.processors.push("speech"),b&&this.processors.forEach(function(c){a(c,b)})}catch(c){console.error(c.name+": "+c.message),sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(1)})}};
...
"Include generated speech in enriched MathML (with -m option only).",c("speech"),"none").option("-w, --structure","Include structure attribute in enriched MathML (with -m option only).",c("structure")).option("-P, --pprint","Pretty print output whenever possible.",c("pprint")).option("-v, --verbose","Verbose mode.").option("-l, --log [name]","Log file [name].").option("--opt","List engine setup options.").on("option:opt",goog.bind(function(){this.enumerate();sre.SystemExternal.process.exit(0)},this)).parse(sre.SystemExternal.process.argv);
